...
:reg_backup
    cls
    echo Exporting your registry settings...
...
:disable_spy
    ...
    echo 1. Stopping and disabling services:
        echo "Stopping and disabling Device Management Push Router Service"
        net stop dmwappushservice 
        sc config dmwappushservice start=disabled
        echo "Stopping and disabling Diagnostics Hub Standard Collector Service"
        net stop diagnosticshub.standardcollector.service
        sc config diagnosticshub.standardcollector.service start=disabled
        echo "Stopping and disabling Windows Error Reporting Service"
        net stop WerSvc
        sc config WerSvc start=disabled
        echo "Stopping and disabling Program Compatibility Assistance Service"
        net stop PcaSvc
        sc config PcaSvc start=disabled
        echo "Stopping and disabling Delivery Optimization Service"
        net stop DoSvc
        sc config DoSvc start=disabled
        echo "Stopping and disabling Windows Media Player Network Sharing Service"
        net stop WMPNetworkSvc
        sc config WMPNetworkSvc start=disabled
        echo "Stopping and disabling Remote Registry Service"
        net stop RemoteRegistry
        sc config RemoteRegistry start=disabled
        echo "Stopping and disabling Terminal Services"
        net stop TermService
        sc config TermService start=disabled
        echo "Stopping and disabling Distributed Link Tracking Client"
        net stop TrkWks
        sc config TrkWks start=disabled
        echo "Stopping and disabling Data Protection Service"
        net stop DPS
        sc config DPS start=disabled
    echo 2. Modifying registry entries:
        echo "Modifying registry entries related to telemetry, privacy, and other settings"
    ...
    echo 3. Ending and disabling scheduled tasks:
